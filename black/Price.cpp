...
namespace BS {
	...
	Price::Price(const double& Expiry,
		const double& Strike,
		const double& Spot,
		const double& Vol,
		const double& r)
	...
	const double Price::calcPV() {
		return calcStandardCallPrice( _tau , _strike , _init , _vol , _r );
	};

	const double Price::calibrateVol(const double& initVol, const double& targetPrice, const double& precision) {
	...
	const double Price::derivVol(const double initVol , const double precision) {
	...
	*/
	...
	const double Price::calcStandardCallPrice(
		const double Expiry,
		const double Strike,
		const double Spot,
		const double Vol,
